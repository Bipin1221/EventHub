...
class PaymentOrder(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    event = models.ForeignKey(Events, on_delete=models.CASCADE)
    ticket_type = models.CharField(max_length=20, choices=[('vip', 'VIP'), ('common', 'Common')])
...
class Notification(models.Model):
    """Notification object for both Attendee and Organizer."""
    recipient = models.ForeignKey(User, on_delete=models.CASCADE, related_name='notifications')
    event = models.ForeignKey(Events, on_delete=models.CASCADE, related_name='event_notifications')
    message = models.TextField()
    is_read = models.BooleanField(default=False)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f'Notification for {self.recipient.name} - {self.event.title}'